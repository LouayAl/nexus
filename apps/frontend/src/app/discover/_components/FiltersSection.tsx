  ...
  return (
    <section ref={resultsRef} style={{ padding:"0 0 56px", scrollMarginTop:80 }}>
      <div style={{ maxWidth:1400, margin:"0 auto", padding:"0 var(--page-px)" }}>
        ...
        <div style={{ display:"flex", gap:8, marginBottom:14, overflowX:"auto", paddingBottom:4, scrollbarWidth:"none", WebkitOverflowScrolling:"touch" } as any}>
          ...
          {CATEGORIES.map(cat => {
            ...
            return (
              <button key={cat.label} onClick={() => { onCategory(active ? null : cat.filter); scrollToResults(); }} style={{
                ...
                padding:"8px 16px", borderRadius:99, fontSize:"clamp(11px,2.5vw,13px)", fontWeight:600,
        ...
        <div style={{ display:"flex", gap:8, marginBottom:24, alignItems:"center", flexWrap:"wrap" }}>
          <span style={{ fontSize:12, fontWeight:600, color:"#5A7A96", marginRight:4, flexShrink:0 }}>Contrat :</span>
        ...
        {loading ? (
          ...
          <div style={{ textAlign:"center", padding:"80px 0" }}>
            ...
            <div className="font-display" style={{ fontSize:"clamp(18px,4vw,22px)", fontWeight:700, color:"#10406B", marginBottom:8 }}>Aucune offre trouvée</div>
            <div style={{ color:"#5A7A96", fontSize:14 }}>Essayez d'autres mots-clés ou filtres</div>
          </div>
        ) : (
          <div className="stagger-children" style={{ display:"grid", gap:16, gridTemplateColumns:"repeat(auto-fill, minmax(min(280px, 100%), 1fr))" }}>
            {offres.map(o => (
