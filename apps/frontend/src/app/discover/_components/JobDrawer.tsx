// frontend/src/app/discover/_components/JobDrawer.tsx
"use client";
...
import {
  ...
  ArrowRight, Heart, Globe, Users, UserCheck,
} from "lucide-react";
...
export function JobDrawer({ offre, onClose }: Props) {
  ...
  const salary   = offre.salaire_visible !== false && offre.salaire_min && offre.salaire_max
    ? `${Math.round(offre.salaire_min / 1000)}K – ${Math.round(offre.salaire_max / 1000)}K MAD / mois`
    : null;
  const skills   = offre.competences?.map(c => c.competence) ?? [];

  // Meta chips at bottom
  const metaChips = [
    { label: "Contrat",     value: offre.type_contrat,        icon: <Briefcase size={11} /> },
    { label: "Expérience",  value: offre.niveau_experience,   icon: <BadgeCheck size={11} /> },
    { label: "Postulants",  value: offre._count?.candidatures != null ? String(offre._count.candidatures) : null, icon: <Users size={11} /> },
    { label: "Langues",     value: offre.langues?.length ? offre.langues.join(", ") : null, icon: <Globe size={11} /> },
  ].filter(m => m.value);
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{
          ...
          {/* ── Header ── */}
          <div style={{ padding: "24px 24px 20px", borderBottom: "1px solid rgba(16,64,107,0.07)", flexShrink: 0 }}>
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: "20px 24px", display: "flex", flexDirection: "column", gap: 22 }}>
            ...
            {salary && (
              <div style={{
                background: "linear-gradient(135deg, rgba(34,132,192,0.06), rgba(16,64,107,0.03))",
                borderRadius: 14, padding: "16px 18px",
                border: "1px solid rgba(34,132,192,0.12)",
                display: "flex", alignItems: "center", gap: 14,
              }}>
                <div style={{
                  width: 40, height: 40, borderRadius: 11, flexShrink: 0,
                  background: "linear-gradient(135deg, #2284C0, #10406B)",
                  display: "flex", alignItems: "center", justifyContent: "center",
                }}>
            ...
            {skills.length > 0 && (
              <div>
                <SectionLabel icon={<BadgeCheck size={11} />} label="Compétences" />
                <div style={{ display: "flex", gap: 7, flexWrap: "wrap" }}>
                  {skills.map(c => (
                    <span key={c.id} style={{
                      background: "#F0F4F8", color: "#2284C0",
                      fontSize: 12, fontWeight: 600, padding: "4px 12px",
                      borderRadius: 8, border: "1px solid rgba(34,132,192,0.15)",
                    }}>
                      {c.nom}
                    </span>
                  ))}
            ...
            {/* ── 4. Meta chips: Contrat / Expérience / Postulants / Langues ── */}
            <div>
              <SectionLabel icon={<Globe size={11} />} label="Informations" />
              <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                {metaChips.map(m => (
                  <div key={m.label} style={{
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", color: "#8FA8BE", marginBottom: 5 }}>
                      {m.icon} {m.label}
                    </div>
          ...
          {/* ── Footer ── */}
          <div style={{ padding: "16px 24px", borderTop: "1px solid rgba(16,64,107,0.07)", display: "flex", gap: 10, flexShrink: 0 }}>
