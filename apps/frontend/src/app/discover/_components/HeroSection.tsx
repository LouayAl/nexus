// apps/frontend/src/app/discover/_components/HeroSection.tsx
"use client";
...
export function HeroSection({ keyword, location, total, onKeyword, onLocation, onSearch, onTag }: Props) {
  return (
    <section style={{ position:"relative", overflow:"hidden", minHeight:520 }}>
      ...
      <div style={{ position:"relative", zIndex:1, maxWidth:1400, margin:"0 auto", padding:"80px 48px 90px", display:"flex", flexDirection:"column", alignItems:"center", textAlign:"center" }}>
        ...
        <h1 className="font-display" style={{ fontSize:"clamp(36px,5vw,60px)", fontWeight:900, color:"white", lineHeight:1.1, marginBottom:18, letterSpacing:"-0.025em", maxWidth:720 }}>
          Votre prochain emploi<br/>
          <span style={{ color:"#EE813D", fontStyle:"italic" }}>vous attend.</span>
        </h1>
        <p style={{ fontSize:17, color:"rgba(255,255,255,0.65)", lineHeight:1.75, marginBottom:44, maxWidth:500 }}>
          Connectez talent et opportunité. Des milliers d'offres sélectionnées pour votre prochaine grande étape.
        </p>

        {/* Search box */}
        <div style={{ background:"white", borderRadius:18, padding:8, display:"flex", width:"100%", maxWidth:920, boxShadow:"0 24px 64px rgba(0,0,0,0.25)" }}>
          ...
          <button
            onClick={onSearch}
            style={{ background:"linear-gradient(135deg, #EE813D, #d4691f)", border:"none", borderRadius:12, padding:"0 32px", color:"white", fontSize:15, fontWeight:700, cursor:"pointer", display:"flex", alignItems:"center", gap:8, fontFamily:"'DM Sans',sans-serif", boxShadow:"0 4px 16px rgba(238,129,61,0.4)", transition:"opacity 0.2s", whiteSpace:"nowrap", minWidth:150 }}
