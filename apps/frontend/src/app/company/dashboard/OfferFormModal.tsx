...
import { ArrowRight, Loader2, CheckCircle, Eye, EyeOff, Building2 } from "lucide-react";
...
export function OfferFormModal({ offre, onClose }: OfferFormModalProps) {
  ...
  const [form, setForm] = useState<FormData>(() => ({
    ...
    entreprise_visible: (offre as any)?.entreprise_visible ?? true,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const payload: CreateOffreDto & { entreprise_visible: boolean } = {
        titre:              form.titre,
      ...
      if (isEdit) {
        await offresApi.update(offre!.id, payload as any);
        toast.success("Offre mise à jour !");
      } else {
        await offresApi.create(payload as any);
    ...
    <Modal title={isEdit ? "Modifier l'offre" : "Créer une nouvelle offre"} onClose={onClose}>
      <form onSubmit={handleSubmit}>
          ...
          {!form.entreprise_visible && (
            <div style={{
              ...
              <span style={{ fontSize: 12, color: "#D64045", fontWeight: 600 }}>
                Les candidats verront "Confidentiel" à la place du nom de votre entreprise.
              </span>
            </div>
          )}
        </div>
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          {form.salaire_min && form.salaire_max && (
            <div style={{
              ...
              background: form.salaire_visible ? "rgba(26,158,111,0.06)" : "rgba(214,64,69,0.05)",
              ...
              <span style={{ fontSize: 12, fontWeight: 600, color: form.salaire_visible ? "#1A9E6F" : "#D64045" }}>
        ...
        <Field label="Compétences requises (séparées par virgule)">
          <input style={inputStyle} value={form.competences_str} onChange={handleChange("competences_str")}
            placeholder="React, TypeScript, Node.js…" />
          {competenceTags.length > 0 && (
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginTop: 8 }}>
              {competenceTags.map(s => (
                <span key={s} style={{ background: "rgba(34,132,192,0.1)", color: "#2284C0", fontSize: 11, fontWeight: 600, padding: "3px 10px", borderRadius: 8 }}>
                  {s}
                </span>
              ))}
            </div>
        ...
        <button type="submit" disabled={isLoading} style={{
          ...
          boxShadow: isLoading ? "none" : "0 4px 20px rgba(16,64,107,0.25)", marginTop: 8,
        }}>
          {isLoading
