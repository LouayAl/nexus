...
const MOCK_NOTIFS: Notification[] = [
  {
    ...
    utilisateurId: 0,
    titre: "Candidature",
    message: "Votre candidature chez TechNova a ete retenue pour un entretien",
  ...
  },
  {
    id: 2,
    utilisateurId: 0,
    titre: "Profil consulte",
    message: "DataVision a consulte votre profil",
    createdAt: new Date().toISOString(),
    lu: false,
  },
  {
    ...
    utilisateurId: 0,
    titre: "Rappel",
    message: "Rappel : entretien avec PixelForge demain a 14h",
    createdAt: new Date().toISOString(),
    lu: true,
  },
  {
    ...
    message: "Nouvelle offre correspondant a votre profil : DevOps chez CloudBase",
    createdAt: new Date().toISOString(),
    lu: true,
  },
];
...
export function NotificationsPanel({ onClose }: { onClose: () => void }) {
  ...
  const markAll = () => setNotifs((items) => items.map((item) => ({ ...item, lu: true })));
  const markOne = (id: number) =>
    setNotifs((items) => items.map((item) => (item.id === id ? { ...item, lu: true } : item)));
    ...
    >
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {notifs.map((notification) => (
          <Card
            key={notification.id}
            style={{ padding: "14px 16px", opacity: notification.lu ? 0.55 : 1, position: "relative", cursor: "pointer", transition: "opacity 0.3s" }}
            onClick={() => !notification.lu && markOne(notification.id)}
          >
            {!notification.lu && (
            ...
            <div style={{ color: theme.text, fontSize: 13, lineHeight: 1.55, marginBottom: 6 }}>{notification.message}</div>
